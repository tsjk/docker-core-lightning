...
CLNRest, which RTL depends on, need not be exposed if it's not needed for something
else. By default its server port is pre-configured to be on 3010. This port is also
user configurable using the environment variable `CLNREST_PORT`.

...
## CLNRest & Ride The Lightning
CLNRest & Ride The Lightning are automatically set up.
...
CLNRest, which RTL depends on, need not be exposed if it's not needed for
something else. By default its server port is pre-configured to be on 3010.
This port is also user configurable using the environment variable
`CLNREST_PORT`.
...
# Waiting for signals after Core Lightning terminates
Sometimes one wish to keep the container running after Core Lightning exits
(unexpectedly). By setting `WAIT_AFTER_CLN_TERMINATION` to true then
`entrypoint.sh` script waits for signals after unexpected Core Lightning
terminations, thus enabling quick restarts of the daemon after relatively
harmless fatal errors. This is only supported when `START_IN_BACKGROUND` is set
to true.
